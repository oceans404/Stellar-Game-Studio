import { Client as RpsClient, type Game } from './bindings';
...
export class RpsService {
  ...
  /**
   * Submit a Rock, Paper, or Scissors move
   */
  async submitMove(
    sessionId: number,
    playerAddress: string,
    move: {tag: 'Rock' | 'Paper' | 'Scissors', values: void},
    signer: Pick<contract.ClientOptions, 'signTransaction' | 'signAuthEntry'>,
    authTtlMinutes?: number
  ) {
    const client = this.createSigningClient(playerAddress, signer);
    const tx = await client.submit_move({
      session_id: sessionId,
    ...
    try {
      ...
      if (err instanceof Error && err.message.includes('Transaction failed!')) {
        throw new Error('Transaction failed - check if the game is still active and you haven\'t already moved');
      }
      throw err;
    }
  }
  ...
  async revealWinner(
    ...
    try {
      ...
      if (err instanceof Error && err.message.includes('Transaction failed!')) {
        ...
        throw new Error('Transaction failed - check if both players have moved and the game is still active');
      }
...
// Example: const rpsService = new RpsService(contractId);
